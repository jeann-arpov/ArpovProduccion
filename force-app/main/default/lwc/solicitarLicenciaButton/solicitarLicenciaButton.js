...
export default class SolicitarLicenciaButton extends NavigationMixin(
  LightningElement
) {
  showModal = false;
  loading = false;
  ...
  async init() {
    ...
    this.doRequest(async (_) => {
      ...
      this.marcas = data.marcas
        .filter((m) => m.Nombre_Obtentor__c != "LDC")
        .map((m) => ({
          value: m.Id,
          label: m.Nombre_Obtentor__c,
          cuit: m.Parent ? m.Parent.N_CUIT__c : m.N_CUIT__c
        }));
      ...
      if (this.isPortalComercio) {
        this.account = {
          ...
          title: "Gestionar Licencia para mí",
          subtitle: "",
          record: null
        };
  ...
  hasLicence(datos) {
    return (
      this.licencias.find(
        (l) => l.Cuenta_Obtentor__r.N_CUIT__c == datos.cuit
      ) != null
    );
  }
  ...
  redirectToSolicitarLicencia() {
    ...
    if (type == "e3") {
      ...
      const validCuits = this.isPortalComercio
        ? this.cuitsPortalComercio
        : this.cuitsOtrosPortales;
      const tercero = this.template.querySelector("c-lookup")?.selection[0];

      if (
        this.isPortalComercio &&
        tercero &&
        tercero.record &&
        this.hasLicence(datos) == false
      )
        return this.onError(
          "No posee una licencia de comercio con la marca seleccionada para gestionar a un tercero"
        );
      ...
      if (
        (this.isPortalComercio || this.isPortalObtentor) &&
        tercero &&
        tercero.id
      )
        state.tercero = tercero.id;

      if (
        licenceType == "Comercio" &&
        ["30646328450", "30707285563", "33710831659"].includes(datos.cuit)
      ) {
        return this.onError(
          `No se pueden gestionar licencia de comercio para esta marca`
        );
      }

      if (!validCuits.includes(datos.cuit))
        return this.onError(
          `Error: ${datos.label} no gestiona licencias para aporte genético`
        );
      this[NavigationMixin.Navigate]({
  ...
  get selectLicenseClass() {
    return (
      "license-type slds-m-bottom_x-large" + (this.type ? " " + this.type : "")
    );
  }
  ...
  get cantContinue() {
    return !(
      this.type != null &&
      (this.type != "genetico" || (this.marca && this.licenceType))
    );
  }
  ...
  async search(event) {
    ...
    await searchAccounts(event.detail)
  ...
  setDefaultSelection(e) {
    if (
      ...
      this.template.querySelector("c-lookup").selection.length == 0
    ) {
