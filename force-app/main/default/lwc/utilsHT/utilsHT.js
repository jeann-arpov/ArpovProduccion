...
const LineaCompraVentaMixin = (cls) =>
  class extends NavigationMixin(cls) {
    ...
    refreshPromoPrice(cantidadTotal) {
      this._promoCantidadTotal =
        cantidadTotal == null ? null : Number(cantidadTotal);
    ...
    get tiposDeCompra() {
      const entries = (this.priceBookEntries || []).filter(
        (v) =>
          v.record.Product2.Variedad2__c == this.variedad ||
          (this.hasId && v.label == this.variedad)
      );
      const tiposCompra = entries.reduce(
        (tiposCompra, entry) => [
          ...tiposCompra,
          {
            label: entry.record.Product2.Tipo_de_Compra__c,
            value: entry.record.Product2.Tipo_de_Compra__c
          }
        ],
        []
      );
    ...
    connectedCallback() {
      loadStyle(
        this,
        fontAwesome + "/fontawesome-free-6.1.1-web/css/all.min.css"
      );
    ...
    preCampaignChange(event) {
      ...
      const dt = new Date();
      this.record.Fecha_de_Activacion__c =
        this.preCampaign == "Disponible"
          ? dt.getFullYear() +
            "-" +
      ...
      this.productChange();
      const priceBookEntry = this.priceBookEntries.find(
        (v) =>
          (v.record.Product2.Variedad2__c == this.variedad ||
            (this.hasId && v.label == this.variedad)) &&
    ...
    getFechaActivacion() {
      const dt = new Date();
      const cultivo =
        this.priceBookEntries[0].record.Product2.Variedad2__r.Cultivo__r.Name;
    ...
    init() {
      const priceBookEntry = this.priceBookEntries.find(
        (v) => v.value == this._record.Id_Producto_de_Lista_de_Precio__c
      );
      console.log(
        priceBookEntry,
        this._record.Id_Producto_de_Lista_de_Precio__c,
        this.priceBookEntries
      );
      if (priceBookEntry) {
        this.preCampaign = priceBookEntry.record.Product2.Tipo_de_Compra__c;
        console.log(
          " init priceBookEntry.record.Product2.Tipo_de_Compra__c ",
          priceBookEntry.record.Product2.Tipo_de_Compra__c
        );
      ...
      console.log(
        "log desde init, valor de tipo de pago:",
        this._record.Tipo_de_Pago__c
      );
    ...
    productChange() {
      ...
      console.log(
        "🔍 DEBUG - productChange - Precio_de_Lista__c actual:",
        this._record.Precio_de_Lista__c
      );
      console.log(
        "🔍 DEBUG - productChange - Es_NC__c:",
        this._record.Es_NC__c
      );
      console.log(
        "🔍 DEBUG - productChange - priceBookEntries count:",
        this.priceBookEntries?.length
      );
      ...
      console.log(
        "🔍 DEBUG - productChange - priceBookEntry encontrado:",
        !!priceBookEntry
      );
      ...
      if (priceBookEntry) {
        ...
        if (!this._record.Precio_de_Lista__c || !this.hasId) {
          ...
          console.log(
            "🔍 DEBUG - productChange - Asignando precio de pricebook (nueva línea):",
            this._record.Precio_de_Lista__c
          );
        } else {
          console.log(
            "🔍 DEBUG - productChange - Manteniendo precio guardado:",
            this._record.Precio_de_Lista__c
          );
        }
    ...
    get variedades() {
      // agrupo por label, que solo aparezca 1. Con la checkbox defino si es pre campaña o entrega inmediata
      const variedades = {};

      for (const priceBookEntry of this.priceBookEntries || []) {
        variedades[priceBookEntry.label] = {
          label: priceBookEntry.label,
          value: priceBookEntry.label
        };
      }
    ...
    hasUnsavedChanges() {
      ...
      if (
        String(this._record.Cantidad__c) !== String(this._persistedCantidad)
      ) {
        return true;
      }
      if (
        String(this._record.Precio_de_Lista__c) !==
        String(this._persistedPrecio)
      ) {
        return true;
      }
      return false;
    ...
    dispatchPromoLineChange(immediate = false, evalModal = false) {
      this.dispatchEvent(
        new CustomEvent("promolinechange", {
          bubbles: true,
          composed: true,
    ...
    getActivePriceBookEntry() {
      ...
      return this.priceBookEntries.find(
        (v) =>
          (v.record.Product2.Variedad2__c == this.variedad ||
            (this.hasId && v.label == this.variedad)) &&
          v.record.Product2.Tipo_de_Compra__c == this.preCampaign
    ...
    getPromoLineData() {
      ...
      const listPrice =
        getPriceBookListPrice(priceBookEntry) ??
        resolveBaseListPrice(this._record.Precio_de_Lista__c);
    ...
    checkAutoSave() {
      ...
      console.log(
        "Tipo de Financiamiento antes de guardar:",
    ...
    updatePriceBasedOnTipoCompra() {
      ...
      console.log(
        "🔍 DEBUG - Precio_de_Lista__c del registro:",
        this._record.Precio_de_Lista__c
      );
      ...
      if (this._record.Precio_de_Lista__c) {
        ...
        console.log(
          "🔍 DEBUG - Usando precio guardado del registro:",
          this.precioLista
        );
        this.updatePrice();
    ...
    openFactura(event) {
      this.dispatchEvent(
        new CustomEvent("openfactura", { detail: this.factura })
      );
    }

    get estado() {
      return this.record.Estado__c == "Creada"
        ? "En Curso"
        : this.record.Estado__c;
    }
...
const CompraVentaMixin = (cls) =>
  class extends NavigationMixin(cls) {
    ...
    async init() {
      ...
      this.title =
        (this.puedeEditar ? (this.recordId ? "Editar " : "Nueva ") : "Ver ") +
        this.community;
    ...
    setDataAndItems(data, items) {
      ...
      if (data.condicionesComerciales)
        this.condicionesComerciales = data.condicionesComerciales;
      if (data.pendientesFacturacionId)
        this.pendientesFacturacionListView = data.pendientesFacturacionId;
      ...
      for (const cv of data.cvs || []) {
        const factura = (cv.Attachments || []).find(
          (a) => a.Name == cv.ERPvs__Comprobante__c + ".pdf"
        );
        if (factura) {
      ...
      if (data.oportunidad) {
        for (const linea of data.oportunidad.OpportunityLineItems) {
          const key =
            this.community.toLowerCase() == "venta"
              ? linea.Linea_de_Compra_HT__r.Linea_de_Venta_HT__c
              : linea.Linea_de_Compra_HT__c;
    ...
    puedeFacturarEnFecha(lineItem) {
      return (
        (
          lineItem.Product2.Variedad2__r.Obtentor_Comercializa__r
            .Dias_de_Exclusion_de_Facturacion_SE__c || ""
        )
          .split(";")
          .map((v) => parseInt(v))
          .includes(new Date().getDate()) == false
      );
    }

    get puedeCrearCvs() {
      return (
        this.data.oportunidad &&
        this.data.oportunidad.StageName == "Pendiente de Facturación" &&
        this.data.oportunidad.OpportunityLineItems.some(
          (l) =>
            l.Product2.Variedad2__r.Obtentor_Comercializa__r
              .Factura_por_Cuenta_y_Orden_SE__c &&
            this.puedeFacturarEnFecha(l) &&
            l.Estado__c == "Facturable" &&
            this.hasNoPCV(l.Id)
        )
      );
    }
    ...
    get puedeCrearFEs() {
      return (
        this.data.cvs &&
        this.data.cvs.some((cv) => cv.ERPvs__Contabilizar__c != true)
      );
    }
    ...
    updateVariedades(records) {
      ...
      for (const record of records) {
        const key =
          record.Product2.Variedad2__r.Obtentor_Comercializa__r.Id_Obtentor__c;
        byObtentor[key] = byObtentor[key] || [];
        byObtentor[key].push({
          label: record.Product2.Nombre_Comercial__c,
          value: record.Id,
          record
        });
      ...
      this.semilleros = Object.keys(byObtentor)
        .map((key) => ({
          value: key,
          label:
            byObtentor[key][0].record.Product2.Variedad2__r
              .Obtentor_Comercializa__r.Nombre_Obtentor__c
        }))
        .sort((a, b) => {
      ...
      this.tiposDeCompra = Array.from(tiposDeCompra).map((value) => ({
        value,
        label: value
      }));

      console.log(
        JSON.parse(JSON.stringify([records, byObtentor, this.semilleros]))
      );
    }
    ...
    applyRecordData(detail, unsavedItems = []) {
      ...
      this.items = this.items.map((item, index) => {
        if (currentItems[index] && currentItems[index].record.Tipo_de_Pago__c) {
          item.record.Tipo_de_Pago__c =
            currentItems[index].record.Tipo_de_Pago__c;
        }
        return item;
      });
    ...
    scheduleRecordPostUpdate() {
      setTimeout(() => {
        ...
        if (this._promoEvalAfterLineRemove) {
          ...
          typeof this.evaluarCondicionPromocionalHtFutura === "function"
        ) {
          this.evaluarCondicionPromocionalHtFutura();
        }
    ...
    updateRecord(event) {
      console.log(
        "[DEBUG] updateRecord - event detail:",
      ...
      if (event.detail) {
        ...
        const unsavedItems = currentItems.filter(
          (item) => item.record && !item.record.Id && item.record.Producto__c
        );
    ...
    get isChildrenLoading() {
      for (const child of this.template.querySelectorAll(
        "c-crear-linea-venta,c-crear-linea-compra"
      )) {
    ...
    get descripcionHtFuturas() {
      return this.cultivoName == "TRIGO"
        ? this.labels.htFuturasTrigo
        : this.labels.htFuturas;
    }
    ...
    openCondicionesComerciales(event) {
      const document = this.condicionesComerciales.find((doc) =>
        doc.Title.includes("_" + this.semillero)
      );
      ...
      if (document) {
        ...
        this.info(
          "No existe pdf de condiciones comerciales para el obtentor " +
            this.semillero
        );
      }
    }
    ...
    async facturar(event) {
      ...
      try {
        ...
        if (this.puedeCrearFEs) {
          for (const cv of this.data.cvs.filter(
            (cv) => cv.Erpvs__Contabilizar__c != true
          )) {
    ...
    get obtentorEsPuntoDeVenta() {
      ...
      return (
        entries?.length > 0 &&
        !entries[0]?.record?.Product2?.Variedad2__r?.Obtentor_Comercializa__r
          ?.Factura_por_Cuenta_y_Orden_SE__c
      );
    }
    ...
    get missingConfirmData() {
      return (
        !this.semillero ||
        (this.community.toLowerCase() == "venta" && !this.productor)
      );
    }
    ...
    redirectPendientesFacturacion() {
      this.redirectToList(
        this.community.toLowerCase() == "venta"
          ? "Venta_HT__c"
          : "Compra_HT__c",
        this.pendientesFacturacionListView
      );
    }
    ...
    redirectMisLicencias() {
      if (this.community.toLowerCase() == "compra") {
        ...
        const pageName = this.isPortalObtentor
          ? "consulta-de-licencias-del-licenciatario"
          : "consulta-de-licencias-del-productor";
