...
export default class CuentaGranariaNew extends LightningElement {
  ...
  tecnologias = {
    ...
    TRIGO: [{ label: "Aporte Genético", value: "RR1/RR2 - BT/Convencional" }],
    CEBADA: [{ label: "Convencional", value: "RR1/RR2 - BT/Convencional" }]
  };
  ...
  verificarAdhesion() {
    doRequest.call(this, async (_) => {
      ...
      if (this.adhesion && Array.isArray(this.adhesion)) {
        const campanas = this.adhesion
          .map((a) => a.Parametro_PPH__r?.Campana__c)
          .filter((c) => c);
  ...
  cargarCampanas(cultivoId) {
    doRequest.call(this, async (_) => {
      const result = await getCampanasConTotales({
        cultivoId,
        tecnologias: this.biotecnologia
      });
  ...
  get decoratedCampanas() {
    ...
    return sorted.map((c, idx) => {
      ...
      const hasAdhesion =
        campanasConAdhesion.has(c.id) || c.pphAdherida === true;
      if (hasAdhesion) {
      ...
      return {
        ...
        cssClass:
          "campana-item" +
          (this.campanaSeleccionada === c.id ? " selected" : "") +
          (isLast ? " campana-activa" : " campana-disabled") +
          (isDisabled ? " campana-disabled" : ""),
        isLast,
        showPPH,
  ...
  getData() {
    doRequest.call(this, async (_) => {
      const results = await getToneladasByCampana({
      ...
      for (const tn of results) {
        ...
        if (key == "Semilla Original" || key == "Semilla Pre Básica")
          key = "Compra SF";
      ...
      for (const origen in this.origenLabels) {
        if (
          !["BolsaTech", "Balance Anual", "Transferencia"].includes(origen) &&
          !totales.hasOwnProperty(origen)
        ) {
  ...
  descargarDetalle(event) {
    ...
    const ids = this.toneladas
      .filter((tn) => tn.Origen__c == origen)
      .map((tn) => tn.Id);
    ...
    if (ids.length) {
      doRequest.call(this, async (_) => {
        ...
        downLink.download = `detalle_toneladas_${origen}.csv`
          .toLowerCase()
          .replace(" ", "_");
        downLink.click();
      });
      ...
      this.dispatchEvent(
        warningEvent(new Error("No hay registros de toneladas para descargar"))
      );
    }
  }
  ...
  get showCampanaSelector() {
    return (
      this.cultivo &&
      this.biotecnologia &&
      this.campanas &&
      this.campanas.length > 0
    );
  }
  ...
  get biotecnologias() {
    console.log(
      "Tecnologías disponibles para cultivo",
      this.cultivoName,
      ":",
      this.cultivoOptions.find((c) => c.value == this.cultivo).label
    );
  ...
  get totalTransferencia() {
    return this.getTotal("Transferencia");
  }
  ...
  get totalRegaliaEnlistPagada() {
    const value = this.toneladas
      .filter(
        (tn) => tn.Origen__c == "Regalía Enlist" && tn.Etapa__c == "Cobrada"
      )
  ...
  get totalTodo() {
    return (
      this.totales.reduce((total, t) => (total += t.value), 0) +
      this.totalBolsatech.value +
      this.totalRegaliaEnlistPagada.value +
      ...
      this.toneladas
        .filter(
          (tn) =>
            ...
            this.toLocalDate(tn.Fecha_Inicio__c).toISOString() >=
              this.fechaEntregadas.toISOString()
        )
  ...
  get totalToneladasRegularizar() {
    const total = this.toneladas.reduce(
      (total, tn) => (total += tn.Cantidad_con_Signo__c),
      0
    );
  ...
  get totalRegaliaEnlistPendientes() {
    return this.toneladas
      .filter(
        (tn) => tn.Origen__c == "Regalía Enlist" && tn.Etapa__c == "Facturada"
      )
      .reduce((total, tn) => (total += tn.Cantidad_con_Signo__c), 0);
  }
