...
export default class HtModals extends NavigationMixin(LightningElement) {
  ...
  icons = {
    error:
      SVG_ICONS +
      "/iconos/popup/Icon-feather-alert-circle.svg#Icon_feather-alert-circle",
    facturas:
      SVG_ICONS + "/iconos/venta/Icon-awesome-receipt.svg#Icon_awesome-receipt",
    licencias:
      SVG_ICONS + "/iconos/venta/Icon-feather-file.svg#Icon_feather-file"
  };
  ...
  get showFinalizada() {
    return (
      this.currentModal == "finalizada" || this.currentModal == "pendiente"
    );
  }
  ...
  get finalizadaSubText() {
    ...
    return (
      prefix +
      ` Por cualquier duda, no dejes de contactarte con nosotros:

        Whatsapp: +54 9 11 3117-2022
        E-mail: ${MESA_AYUDA_EMAIL}
  ...
  get vigenciaText() {
    return (
      "Las " +
      this.community.toLowerCase() +
      "s tienen una vigencia de 48hs iniciado el proceso, una vez cumplidas las 48hs la misma caduca y se deberá volver a iniciar el proceso"
    );
  }

  get licenciasLabel() {
    return this.community.toLowerCase() == "venta"
      ? "Ir a consulta de licencias"
      : "Ir a mis licencias";
  }
  ...
  selectContado() {
    this.dispatchEvent(
      new CustomEvent("tipopagoselected", { detail: { value: "Contado" } })
    );
  }

  selectFinanciado() {
    this.dispatchEvent(
      new CustomEvent("tipopagoselected", { detail: { value: "Financiado" } })
    );
  }
}
