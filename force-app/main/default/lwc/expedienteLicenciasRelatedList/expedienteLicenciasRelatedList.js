...
export default class ExpedienteLicenciasRelatedList extends NavigationMixin(
  LightningElement
) {
  @api recordId;
  ...
  wiredContactId({ error, data }) {
    if (data) {
      ...
      // eslint-disable-next-line no-console
      console.error(
        "[expedienteLicenciasRL] Error loading user ContactId:",
        error
      );
    }
  }
  ...
  get effectiveExpedienteId() {
    ...
    if (ref) {
      ...
      const fromState = [state.recordId, state.c__recordId, state.id].find(
        (v) => this.isSalesforceId(v)
      );
      if (fromState) {
        return fromState;
      }
  ...
  wiredLicencias(result) {
    ...
    if (result.data) {
      ...
      this.totalRecords =
        result.data.totalRecords != null ? result.data.totalRecords : 0;
      this.totalPages =
        result.data.totalPages != null ? result.data.totalPages : 0;
  ...
  handleOpenLicencia(event) {
    ...
    console.log(
      "[expedienteLicenciasRL] Opening portal for license:",
      licenseId
    );
    console.log("[expedienteLicenciasRL] Current User ID:", this.currentUserId);
    console.log(
      "[expedienteLicenciasRL] Current Contact ID:",
  ...
  get mostrarVacío() {
    return (
      !this.loading && !this.hayFilas && !this.loadError && !this.errorMessage
    );
  }
  ...
  get disableNext() {
    return (
      this.pageNumber >= this.totalPages || this.loading || this.totalPages < 1
    );
  }
