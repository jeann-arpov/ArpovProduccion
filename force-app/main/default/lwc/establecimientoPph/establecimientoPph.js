...
export default class EstablecimientoPph extends NavigationMixin(
  LightningElement
) {
  @api info;
  @api hiding;
  ...
  get titulo() {
    return (
      "Establecimiento " +
      (this.establecimiento?.Name || (this.name ? " - " + this.name : ""))
    );
  }
  ...
  updateCantidad(event) {
    this.dispatchEvent(
      new CustomEvent("updatecantidad", { detail: event.detail })
    );
  }
  ...
  updateLocation(data, map) {
    ...
    if (this.validateCoordinates(data.longitude, data.latitude)) {
      ...
      this.dispatchEvent(
        errorEvent(new Error("Las coordenadas deben ser negativas"))
      );
    }
  }
  ...
  get totalSembrado() {
    return (
      this.safeCantidadNoSE +
      this.variedadesPPH
        .map((e) => e.getData().cantidad)
        .reduce((a, b) => a + b, 0)
    );
  }
  ...
  validate() {
    ...
    if (
      this.grandesCuentas == false &&
      !this.establecimiento &&
      this.mapCoodinates.startsWith("Selec")
    ) {
      this.template
  ...
  @api getData() {
    ...
    let data = {
      ...
      longitude:
        this.establecimiento?.Coordenadas__Longitude__s || this.longitude,
