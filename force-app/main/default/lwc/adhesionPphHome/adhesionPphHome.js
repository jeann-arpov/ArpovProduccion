...
export default class AdhesionPphHome extends NavigationMixin(LightningElement) {
    ...
    try {
      ...
      this.data.forEach((w) => {
        w.parametros.forEach((wParam) => {
          ...
          wParam.estadoClass =
            "estado " + this.getEstadoClass(wParam.planSiembra?.Estado__c);
          wParam.estadoLabel = wParam.planSiembra
            ? this.getEstadoLabel(wParam.planSiembra.Estado__c)
            : "Sin Adherir";
          wParam.showRectificacionInfo =
            wParam.planSiembra?.Estado__c === "Rectificado";
          wParam.estadoWrapperClass = wParam.showRectificacionInfo
            ? "estado-tooltip"
            : "estado-wrapper";
  ...
  async doAction(event) {
    ...
    if (
      ...
      wParam.actionName == "Continuar" ||
      wParam.actionName == "Ver"
    ) {
  ...
  getActionName(wParam) {
    ...
    if (estado == "Adherido" || estado == "Rechazado" || estado == "Vencido" || estado == "En Revisión") {
      actionName = "Ver";
    }
  ...
  showTerminos(event) {
    ...
    const documentId = this.parametros.find(
      (w) => w.parametro.Id == id
    ).contentDocumentId;
