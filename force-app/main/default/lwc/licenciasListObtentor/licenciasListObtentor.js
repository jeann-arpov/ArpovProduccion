 ...
 */
export default class LicenciasListObtentor extends NavigationMixin(
  LightningElement
) {
  ...
  async loadFilters() {
    try {
      ...
      const promises = [getEstadosLicencia(), getTiposLicencia()];
      ...
      const [estadosData, tiposData, origenesData] =
  ...
  async handleCall(event) {
    ...
    console.warn(
      "[LicenciasListObtentor - Reporte] handleCall: click recibido"
    );
    ...
    try {
      ...
      console.error(
        "[LicenciasListObtentor - Reporte] Error en handleCall:",
        error
      );
    }
  }
