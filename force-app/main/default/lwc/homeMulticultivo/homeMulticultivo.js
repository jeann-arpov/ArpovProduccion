...
export default class HomeMulticultivo extends LightningElement {
  ...
  doAction(event) {
    const tarjeta = this.tarjetas.find(
      (t) => t.cultivo.Id == event.target.dataset.id
    );

    if (tarjeta.isComprar) {
      window.open(
        `${SITE_URL}s/editar-compra?cultivoId=${tarjeta.cultivo.Id}`,
        "_parent"
      );
    }
    ...
    if (tarjeta.isAdherir) {
      window.open(
        `${SITE_URL}s/adhesion-pph?recordId=${tarjeta.paramId}`,
        "_parent"
      );
    }
  }
  ...
  redirectToCC(event) {
    const cultivo = this.tarjetas.find(
      (t) => t.cultivo.Id == event.target.dataset.id
    ).cultivo;
    window.open(
      `${SITE_URL}s/cuenta-granaria?cultivoId=${cultivo.Id}`,
      "_parent"
    );
  }
