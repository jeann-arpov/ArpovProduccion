...
export default class CesionPph extends NavigationMixin(LightningElement) {
  ...
  get pageRecordId() {
    if (window.location.href.includes("s/cesion-ht/"))
      return window.location.href.split("s/cesion-ht/")[1].split("/")[0];
    if (window.location.href.includes("recordId="))
      return window.location.href.split("recordId=")[1].split("&")[0];
    return null;
  }
  ...
  loadData(data) {
    if (data.cultivos) {
      ...
      if (data.cesion) {
        ...
        this.variedades.forEach(
          (v) => (v.totals = { stock: data.stock[v.Id] })
        );
      ...
      for (const destinatario of data.destinatarios) {
        const dest = {
          ...
          record: destinatario,
          lineas: [],
          account: this.accounts[destinatario.Destinatario__c]
        };

        for (const variedadId of Object.keys(variedades)) {
          const record =
            (destinatario.Lineas_Cesion_HT__r || []).find(
          ...
          for (const linea of destinatario.Lineas_Cesion_HT__r || []) {
            if (!dest.lineas.find((l) => l.id == linea.Variedad__c)) {
              dest.lineas.push({
                ...
                record: linea,
                variedad: { ...linea.Variedad__r, totals: { stock: 0 } }
              });
            }
          }
        }
  ...
  remove(event) {
    if (this.destinatarios.length == 1)
      return this.onError("No puede borrar el único destinatario restante");
  ...
  async startCesion() {
    await this.doRequest(async (_) => {
      const data = await getNewCesionData({
  ...
  redirectToCompraHT() {
    this[NavigationMixin.GenerateUrl]({
      ...
      window.open(`${url}?cultivoId=${this.cesion.Cultivo__c}`, "_self")
    );
  }
  ...
  isValid(showError = false, isContinue = false) {
    ...
    try {
      for (const destinatario of this.template.querySelectorAll(
        "c-destinatario-pph"
      )) {
  ...
  async save() {
    ...
    await this.doRequest(async (_) => {
      if (this.isValid()) {
        ...
        const toSend = {
          destinatarios: data.destinatarios.filter(
            (d) =>
              d.record?.Estado__c == "En Curso" || d.record?.Estado__c == null
          )
        };
        console.log(JSON.parse(JSON.stringify(toSend)));
        const newData = await save({
          js: JSON.stringify(toSend),
  ...
  get data() {
    ...
    for (const destinatario of this.template.querySelectorAll(
      ...
      if (dest.destinatarioId)
        this.accounts[dest.destinatarioId] = destinatario.getAccount(); //se resetea el componente, me guardo en el map la info de la account para no tener que traer en cada load
    }

    data.stock = this.variedades
      .map((v) => v.totals.stock)
      .reduce((a, b) => a + b, 0);
    console.log(data);
    return data;
  }
  ...
  get hasDestinatarioEnCurso() {
    return (
      this.destinatarios.find((d) => d.record?.Estado__c == "En Curso") != null
    );
  }
  ...
  async edit(id) {
    ...
    await this.doRequest(async (_) => {
      if (!this.isEnCurso)
        await backToEnCurso({ cesionId: this.cesion.Id }).then((data) =>
          this.loadData(data)
        );
  ...
  async anular() {
    ...
    await this.doRequest(async (_) => {
      await anular({ cesionId: this.cesion.Id }).then((data) =>
        this.loadData(data)
      );
    });
  }
  ...
  enviarConfirm(e) {
    if (this.showWarningHT) {
      this.dispatchEvent(
        errorEvent(
          new Error(
            "No se puede enviar la cesión porque no tiene el stock suficiente"
          )
        )
      );
      return;
    ...
    this.currentModal = e.detail.hasLicenses
      ? "confirm-enviar-cesion"
      : "confirm-enviar-cesion-without-licenses";
  }

  async enviar(e) {
    await this.doRequest(async (_) => {
      await sendCesion({ cesionId: this.cesion.Id }).then((data) =>
        this.loadData(data)
      );
  ...
  trackCesionConfirmada() {
    ...
    const cantidad_ht = this.variedades.reduce(
  ...
  get title() {
    if (this.isResumen) {
      return this.cesion.Estado__c == "Pendiente de Validación"
        ? "¡Tu solicitud de cesión ya fue enviada!"
  ...
  get showWarning() {
    return (
      ...
      (this.showWarningFechaCultivo || this.showWarningHT)
    );
  }
  ...
  get showWarningFechaCultivo() {
    const now = Date.now();
    return (
      now < Date.parse(this.fechaInicioCesion) ||
      now > Date.parse(this.fechaFinCesion)
    );
  }

  get fechaInicioCesion() {
    return this.tipoCesion == "Explotacion Conjunta"
      ? this.cesion?.Cultivo__r.Fecha_Inicio_de_Cesion__c
      : this.cesion?.Cultivo__r.Fecha_Inicio_de_Cesion_Semilla_Original__c;
  }

  get fechaFinCesion() {
    return this.tipoCesion == "Explotacion Conjunta"
      ? this.cesion?.Cultivo__r.Fecha_Fin_de_Cesion__c
      : this.cesion?.Cultivo__r.Fecha_Fin_de_Cesion_Semilla_Original__c;
  }
  ...
  get formattedFechaCesion() {
    ...
    return formmatedDate.toLocaleDateString("es-AR", {
      day: "numeric",
      month: "long"
    });
  }
  ...
  handleFilePreview(e) {
    const file = this.files.find(
      (f) => f.ContentDocument.Id == e.target.dataset.id
    );
    window.open(
      window.location.href.split("/s/")[0] +
        "/s/contentdocument/" +
